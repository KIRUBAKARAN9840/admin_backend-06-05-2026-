...
class GooglePlayCreditsProcessor:
    ...
    async def _verify_via_revenuecat(
        ...
        with self._session_scope() as session:
            ...
            def _op() -> Dict[str, Any]:
                ...
                # 3) Determine credits to grant from the order items
                credits_to_grant = 0
                product_sku = None
                items = (
                ...
                for pid, purchases in non_subs.items():
                    if any(sku in pid for sku in CREDIT_PACKS) and purchases:
    ...
    async def _handle_webhook(
        ...
        is_credit_product = any(sku in product_id for sku in CREDIT_PACKS)
    ...
    async def _webhook_grant_credits(
        ...
        sku = None
        for known_sku in CREDIT_PACKS:
            if known_sku in product_id:
                sku = known_sku
                break
        credits_to_grant = CREDIT_PACKS.get(sku, 50) if sku else 50
    ...
    async def _webhook_refund_credits(
        ...
        for known_sku in CREDIT_PACKS:
            if known_sku in product_id:
                sku = known_sku
                break
        credits_to_refund = CREDIT_PACKS.get(sku, 50) if sku else 50
    ...
    def _sync_read_credit_state(
        ...
        with self._session_scope() as session:
            ...
            bal = (
                session.query(CreditBalance)
                .filter(CreditBalance.customer_id == customer_id)
                .first()
            )
            return CreditVerifyResult(
                verified=True,
