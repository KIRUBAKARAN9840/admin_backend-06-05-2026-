...
Mirrors the Google Play processor's table-write logic exactly (Order +
OrderItem + Payment + Entitlement + NutritionEligibility + FittbotPayment +
100 AI credits + 3 reward entries) but adapted for Razorpay (Provider.razorpay_pg,
gateway="razorpay", server-side rzp_create_order on checkout, HMAC signature
verification on the verify call).

Verify uses the dailypass-style pattern (no fallback task, no pending_verify
key, no verify_done flag) — the worker does the work end-to-end with two
fast-path optimisations (capture marker and existing-Payment lookup) and DB
unique constraints as the safety floor.
"""
...
class RazorpayNutritionPackageProcessor:
    ...
    # 100 AI credits + 3 reward entries. NO NutritionBooking.
    # ================================================================
