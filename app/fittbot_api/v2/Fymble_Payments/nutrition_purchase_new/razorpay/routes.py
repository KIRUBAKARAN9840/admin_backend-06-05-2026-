"""
FastAPI router for Razorpay nutrition-package purchases.

Same shape as the Google Play router (pay first, book later) but the verify
step accepts Razorpay payment id + signature returned from the SDK callback.
"""
...
from .dispatcher import RazorpayNutritionPackageDispatcher
from .schemas import (
...
router = APIRouter(
    prefix="/nutrition_purchase_new/razorpay",
    tags=["Nutrition Package Purchase - Razorpay"],
)
...
async def _get_dispatcher(
    ...
    config: HighConcurrencyConfig = Depends(_get_config),
) -> RazorpayNutritionPackageDispatcher:
    return RazorpayNutritionPackageDispatcher(store, config)


# ── Checkout (no slot info needed) ───────────────────────────────────
...
async def enqueue_rp_nutrition_package_checkout(
    request: Request,
    body: RpNutritionPackageCheckoutRequest,
    client_id: int = Depends(get_verified_client_id),
    dispatcher: RazorpayNutritionPackageDispatcher = Depends(_get_dispatcher),
):
    try:
    ...
    status_url = request.url_for(
        "get_rp_nutrition_package_command_status", command_id=record.command_id
    )
    ...
    await track_event(
        ...
        source="payment_nutrition_package_razorpay",
...
async def enqueue_rp_nutrition_package_verify(
    request: Request,
    body: RpNutritionPackageVerifyRequest,
    client_id: int = Depends(get_verified_client_id),
    dispatcher: RazorpayNutritionPackageDispatcher = Depends(_get_dispatcher),
):
...
@router.get(
    ...
    name="get_rp_nutrition_package_command_status",
)
async def get_rp_nutrition_package_command_status(
    command_id: str,
    client_id: int = Depends(get_verified_client_id),
    store: AsyncCommandStore = Depends(_get_async_store),
):
