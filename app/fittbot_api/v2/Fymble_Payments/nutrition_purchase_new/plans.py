"""
Catalog of Fymble nutrition / AI-diet-coach purchase plans, shared by Google
Play and Razorpay processors.

Adding a new plan = add an entry below. Both processors look up by product_sku
and read price, sessions, schedule, validity, bonus credits, plan name, kind,
and reward-entry flag from the matching NutritionPlan instance — no per-plan
branching anywhere except the single "what to grant" switch in _fulfill_order
keyed on plan.kind.

The catalog is also used by the webhook layer to filter incoming events:
RevenueCat events whose product_id is not in the catalog are ignored, and
each webhook fulfills only the order whose OrderItem.sku matches that
product_id — so concurrent purchases of different plans by the same customer
cannot get crossed.
"""
...
_PLANS: Dict[str, NutritionPlan] = {
    ...
    "ai_diet_coach": NutritionPlan(
        ...
        session_schedule=[],
        bonus_credits=100,
        validity_days=36500,  # ~100 years — effectively forever
