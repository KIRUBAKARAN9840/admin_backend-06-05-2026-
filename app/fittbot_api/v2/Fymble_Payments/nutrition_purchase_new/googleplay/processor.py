"""
Background worker for Google Play (RevenueCat) nutrition-package lifecycle.

Plan parameters (price, sessions, schedule, validity, bonus credits, plan name,
reward-entry flag) are resolved per-purchase from `plans.py` keyed by the
product_sku on the request. Adding a new plan only requires editing
plans.py — this processor is plan-agnostic.

Flow:
  purchase  → create pending Order (no slot validation)
  verify    → Phase 1: Redis capture check → pending_webhook + 20s fallback
  fallback  → Phase 2: RevenueCat API call → grant if webhook hasn't
  webhook   → grant + complete pending verify

On verified payment, grants:
  1. Payment record (captured)
  2. Entitlement (nutrition, plan.validity_days)
  3. NutritionEligibility (plan.total_sessions, plan.session_schedule, plan.validity_days)
  4. FittbotPayment ledger
  5. plan.bonus_credits AI credits via CreditService (skipped if 0)
  6. 3 reward program entries via add_nutrition_purchase_entry_sync
     (skipped if plan.grant_reward_entries is False)
  (NO NutritionBooking — client books separately)
"""
...
class GooglePlayNutritionPackageProcessor:
    ...
    async def process_purchase(self, command_id: str, store: CommandStore) -> None:
        ...
        try:
            ...
            logger.exception("Nutrition package purchase command failed: %s", exc)
    ...
    async def process_verify(self, command_id: str, store: CommandStore) -> None:
        ...
        logger.info(
            "NUTRITION_PKG_GP_VERIFY_PENDING_WEBHOOK | customer=%s command=%s fallback_in=%ds",
            _mask(customer_id),
            command_id,
            self.config.gp_nutrition_verify_total_timeout_seconds,
        )
    ...
    async def process_verify_fallback(
        ...
        try:
            ...
            logger.exception("Nutrition package GP verify fallback failed: %s", exc)
    ...
    async def process_webhook(self, command_id: str, store: CommandStore) -> None:
        ...
        try:
            ...
            logger.exception("Nutrition package GP webhook command failed: %s", exc)
            await store.mark_failed(command_id, str(exc))
            return
    ...
    def _fulfill_order(
        ...
        *,
        ...
        plan.bonus_credits AI credits (skipped if 0) → 3 reward entries
        (skipped if plan.grant_reward_entries is False).
        ...
        # ── 7. Grant 3 reward program entries (per-plan flag) ────────
        if plan.grant_reward_entries:
            try:
                reward_ok, entries_added, reward_msg = add_nutrition_purchase_entry_sync(
                    session=session,
                    client_id=int(customer_id),
                    source_id=order.id,
                )
        ...
        logger.info(
            "FYMBLE_PKG_GP_FULFILLED | order=%s customer=%s ent=%s elig=%s "
    ...
    def _complete_pending_verify(
        ...
        logger.info(
            "NUTRITION_PKG_GP_WEBHOOK_COMPLETED_PENDING_VERIFY | command=%s customer=%s",
            command_id, _mask(customer_id),
        )
