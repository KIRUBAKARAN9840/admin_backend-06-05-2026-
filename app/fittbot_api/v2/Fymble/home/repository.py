...
from app.models.nutrition_models import NutritionBooking, ClientDietTemplate
...
class HomeRepository:
    ...
    async def get_initial_state(
        ...
        try:
            ...
            pipe.get(nondp_last_key)       # rotation pointer ("rewards" / "refer" / None)
    ...
    async def fetch_credit_balance(self, client_id: int) -> int:
        """Fetch credit balance for home display. Simple read — no expiry logic."""
    ...
    async def fetch_nutrition_status(self, client_id: int) -> Tuple[bool, bool, bool, Optional[dict], Optional[int]]:
        ...
        Updated for 4-session package flow:
          ...
          Priority 2: Most recent 'attended' session → check diet_plan_assigned
          Priority 3: No relevant bookings → all False

        Uses 180-day cutoff (matching eligibility expiry) instead of 31 days,
        so all 4 sessions across ~4 weeks remain visible.
        ...
        async with AsyncSessionLocal() as session:
            ...
            # Priority 3: No active package — all 4 done or never purchased
