...
class ChatDietService:
    ...
    async def get_current_state(self, client_id: int) -> CurrentPlanResponse:
        """Latest plan + followup eligibility + today's target/consumed, in one shot."""
        ...
        plan_day, target = self._today_target_from_plan(
            plan_record["created_at"], plan_record["plan"],
        )
    ...
    @staticmethod
    def _today_target_from_plan(
        plan_created_at: str, plan: list,
    ) -> tuple:
        """Map today (IST) → which day of the 7-day plan, and return that day's target.

        plan_day = days_since_plan_created + 1, wrapped to 1..7.
        Returns (plan_day, target_calories). Falls back to (1, day1.total) on parse issues.
        """
        plan_day = 1
        try:
            created_date = datetime.fromisoformat(plan_created_at).date()
            days_since = (today_ist() - created_date).days
            if days_since >= 0:
                plan_day = (days_since % 7) + 1
        except (TypeError, ValueError):
            plan_day = 1

        day_block = next((d for d in plan if d.get("day") == plan_day), None)
        if day_block is None and plan:
            day_block = plan[0]
            plan_day = day_block.get("day", 1)
        target = int(day_block.get("total_calories", 0)) if day_block else 0
        return plan_day, target
