...
class TodayInfo(BaseModel):
    """Today's calorie target (from AI plan) and actual consumption (from ActualDiet)."""
    date: str                            # YYYY-MM-DD in IST
    plan_day: Optional[int] = None       # 1-7 — which day of the latest plan today maps to. null if no plan.
    target_calories: int = 0             # AI-set target for today (latest_plan.plan[plan_day-1].total_calories)
    consumed_calories: int = 0           # Sum of today's ActualDiet entries — frontend's existing logic, computed server-side
...
class CurrentPlanResponse(BaseModel):
    ...
    plus today's target vs consumed calories.
    """
    status: int = 200
    has_plan: bool = False
