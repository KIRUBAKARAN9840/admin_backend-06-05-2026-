...
class DailyPassRepository:
    ...
    async def get_user_offer_eligibility(self, client_id: Optional[int]) -> Dict:
        """Check if user is eligible for ₹49 offer. Returns client_name, dp count, eligibility."""
        if not client_id:
            return {
                "dailypass_count": 0,
        ...
        try:
            dp_ineligible_cached = await self.redis.exists(dp_ineligible_key)
        except RedisError as e:
            jlog("warning", {
                "type": "cache_read_failure",
                "error_code": "DP_INELIGIBLE_CACHE_READ",
                "detail": str(e),
                "client_id": client_id,
            })
        ...
        # Daily pass count: skip DB if cached as ineligible
        if dp_ineligible_cached:
            ...
            if dp_count >= 3:
                try:
                    await self.redis.set(dp_ineligible_key, "1")
                except RedisError as e:
                    jlog("warning", {
                        "type": "cache_write_failure",
                        "error_code": "DP_INELIGIBLE_CACHE_WRITE",
                        "detail": str(e),
                        "client_id": client_id,
                    })
