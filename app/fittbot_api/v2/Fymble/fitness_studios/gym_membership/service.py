...
class MembershipService(BaseListingService):
    ...
    async def list_gyms(self, params: MembershipListParams, client_id: int = None) -> MembershipListResponse:
        ...
        # Two-key system: visited key is set on gym_details, walkaway key is
        # promoted here on the listing page (user "walked away" from details)
    ...
    async def get_gym_details(self, gym_id: int, client_id: int = None) -> Optional[GymDetailsResponse]:
        ...
        gym_result = await self.db.execute(
            select(Gym).where(Gym.gym_id == gym_id, Gym.fittbot_verified.is_(True)).limit(1)
        )
        ...
        return GymDetailsResponse(
            data=GymDetailsData(
                ...
                gym_timings=json.loads(gym.gym_timings) if isinstance(gym.gym_timings, str) else gym.gym_timings,
                photos=[
