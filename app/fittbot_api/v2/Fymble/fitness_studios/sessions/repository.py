...
class SessionRepository:
    ...
    async def get_user_offer_eligibility(self, client_id: Optional[int]) -> Dict:
        """Check if user is eligible for ₹99 session offer. Returns client_name, session count, eligibility.

        24h Redis cache (invalidated by session_processor when a paid booking lands)
        eliminates the per-request DB hit on the home hit path.
        """
        if not client_id:
            return {
                "session_count": 0,
        ...
        elig_key = USER_SESSION_OFFER_ELIG_KEY.format(client_id=client_id)

        # Fast path: 24h cache hit — no DB at all
        try:
            cached = await self.redis.get(elig_key)
            ...
            jlog("warning", {
                ...
                "error_code": "SESSION_OFFER_ELIG_CACHE_READ",
                "detail": str(e),
                "client_id": client_id,
            })

        ineligible_key = USER_SESSION_INELIGIBLE_KEY.format(client_id=client_id)

        # Check Redis for permanent ineligibility flag
        ...
        try:
            ...
            jlog("warning", {
                "type": "cache_read_failure",
                "error_code": "SESSION_INELIGIBLE_CACHE_READ",
                "detail": str(e),
                "client_id": client_id,
            })
        ...
        # Session booking count: skip DB if cached as ineligible
        if ineligible_cached:
            ...
            # Cache ineligibility permanently if >= 3
            if session_count >= 3:
                try:
                    ...
                    jlog("warning", {
                        ...
                        "error_code": "SESSION_INELIGIBLE_CACHE_WRITE",
                        "detail": str(e),
                        "client_id": client_id,
                    })
        ...
        # Cache for 24h (invalidated by session_processor on new paid booking)
        try:
            await self.redis.setex(elig_key, CACHE_TTL_USER_OFFER, json.dumps(result))
        except RedisError as e:
            jlog("warning", {
                "type": "cache_write_failure",
                "error_code": "SESSION_OFFER_ELIG_CACHE_WRITE",
                "detail": str(e),
                "client_id": client_id,
            })
