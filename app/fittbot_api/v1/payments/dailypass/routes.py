...
class UnifiedCheckoutRequest(BaseModel):
    ...
    numberOfUsers: int = Field(1, ge=1, le=10)
    selectedTime: Optional[str] = Field(None)

    # Old app fields (backward compat)
    startDate: Optional[str] = Field(None, description="Start date YYYY-MM-DD (old app)")
    daysTotal: Optional[int] = Field(None, ge=1, le=60, description="Number of days (old app)")
    ...
    userType: Optional[str] = Field(None)
    planDetails: Optional[Any] = Field(None)
    is_offer_eligible: Optional[bool] = Field(False)

    @model_validator(mode="after")
    def _backfill_dates(self):
        """Old apps send startDate+daysTotal; compute dates from them."""
        if not self.dates and self.startDate and self.daysTotal:
            start = date.fromisoformat(self.startDate)
            self.dates = [(start + timedelta(days=i)).isoformat() for i in range(self.daysTotal)]
        if not self.dates:
            raise ValueError("Either 'dates' or 'startDate'+'daysTotal' must be provided")
        return self
    ...
    def _v_dates(cls, v):
        ...
        if len(v) > MAX_DAILY_PASS_DAYS:
            raise ValueError(f"Maximum {MAX_DAILY_PASS_DAYS} dates allowed")
        for d in v:
            try:
...
class UnifiedCheckoutResponse(BaseModel):
    ...
    finalAmount: int
    perUserAmount: Optional[int] = None
    # service
    gymId: int
    daysTotal: int
    dates: List[str]
