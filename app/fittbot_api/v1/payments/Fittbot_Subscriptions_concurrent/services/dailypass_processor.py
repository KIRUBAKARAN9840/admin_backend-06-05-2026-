...
from app.services.timezone_utils import UTC, IST
...
from app.models.fittbot_models import ReferralFittbotCash, Client
from app.models.fittbot_plans_model import FittbotPlan

# TODO: local reward logic in this file can be replaced with calculate_reward()
from app.services.reward_calculator import calculate_reward, REWARD_PERCENT
...
async def _check_dailypass_offer_eligibility(db: AsyncSession, client_id: int, gym_id: int) -> tuple:
    """Returns (is_eligible: bool, dp_count: int)."""
...
async def _process_daily_pass_activation_async(
    ...
    num_users = metadata.get("number_of_users", 1)
    booking_type = metadata.get("booking_type", "single")

    logger.info(f"[DAILYPASS_METADATA] gym_id={gym_id}, dates_count={len(dates)}, selected_time={selected_time}, num_users={num_users}")
    ...
    per_user_price = actual_amount_paid // num_users if num_users > 0 else actual_amount_paid

    logger.info(f"Daily pass amount calculation: subtotal={amount_before_rewards}, reward={reward_amount}, final={actual_amount_paid}, per_user={per_user_price}")
    ...
    daily_pass = DailyPass(
        ...
        head_count=num_users,
        per_user_price=per_user_price,
        purchase_timestamp=datetime.now(IST),
    )

    logger.info(f"[DAILYPASS_CREATING] Creating DailyPass: client_id={customer_id}, gym_id={gym_id}, days={len(dates)}, num_users={num_users}, amount={actual_amount_paid}")
    db.add(daily_pass)
    ...
    db.add(
        DailyPassAudit(
            daily_pass_id=daily_pass.id,
            action="purchase",
            details=f"Daily pass purchased for {len(dates)} days, {num_users} user(s)",
...
class DailyPassProcessor:
    ...
    async def _invalidate_home_booking_cache(self, gym_ids: Set[int], client_id: Optional[str] = None) -> None:
        """Clear home booking counts cache for impacted gyms + dailypass eligibility."""
        if not self.redis:
            return
        for gid in (gym_ids or set()):
            if gid is None:
                continue
        ...
        if client_id:
            try:
                await asyncio.to_thread(self.redis.delete, f"user:{client_id}:dp_ineligible")
            except Exception:
                logger.warning("DP_INELIGIBLE_CACHE_DELETE_FAILED", extra={"client_id": client_id})
            # Invalidate bookings list cache
            await self._invalidate_dailypass_bookings_cache(client_id)


    async def _invalidate_dailypass_bookings_cache(self, client_id: str) -> None:
        """Clear cached /api/v2/bookings/dailypass/all response for a client."""
        if not self.redis:
            return
        pattern = f"bookings:dailypass:active:{client_id}:*"
        try:
            cursor = 0
            while True:
                cursor, keys = await asyncio.to_thread(self.redis.scan, cursor, match=pattern, count=100)
                if keys:
    ...
    async def process_checkout(self, command_id: str, store) -> None:
        ...
        pel.checkout_started(command_id=command_id, client_id=str(payload.clientId),
                             gym_id=payload.gymId, days_total=len(payload.dates))
        try:
    ...
    async def fulfill_from_webhook(
        ...
        async with SessionLocal() as db:
            ...
            try:
                ...
                await self._invalidate_home_booking_cache(gym_ids, client_id=order.customer_id)
    ...
    async def _checkout_async(self, db: AsyncSession, payload: UnifiedCheckoutRequest) -> Dict[str, Any]:
        ...
        # Parse dates list from client
        dp_dates = sorted([date.fromisoformat(d) for d in payload.dates])
        days_total = len(dp_dates)
        num_users = payload.numberOfUsers
        ...
        if client_id_int is not None:
            is_offer_eligible, dp_count = await _check_dailypass_offer_eligibility(db, client_id_int, payload.gymId)
            logger.info(
        ...
        # Async DB queries for pricing
        actual_per_day_minor = await _get_price_for_gym_async(db, payload.gymId, False)
        actual_price = await _get_actual_price_for_gym_async(db, payload.gymId, is_offer_eligible)

        # Split pricing: user may have intro offer, friends always pay actual
        friends_count = max(0, num_users - 1)
        dp_pricing = calculate_dailypass_pricing(
            number_of_days=days_total,
            offer_price=OFFER_PRICE_PAISE,
            actual_price=actual_per_day_minor,
            offer_active=is_offer_eligible,
            dp_count=dp_count,
            friends_count=friends_count,
        )

        per_day_minor = dp_pricing["dailypass_price"]
        dp_gross = dp_pricing["total_amount"]

        logger.info("DAILYPASS_PRICING", extra={
            "per_day_minor": per_day_minor, "actual_per_day_minor": actual_per_day_minor,
            "actual_price": actual_price, "dp_gross": dp_gross,
            "days_total": days_total, "num_users": num_users,
            "friends_count": friends_count,
            "user_amount": dp_pricing["user_amount"],
            "friends_amount": dp_pricing["friends_amount"],
            "offer_days_count": dp_pricing["offer_days_count"],
            "show_modal_self": dp_pricing["show_modal_self"],
            "show_modal_friend": dp_pricing["show_modal_friend"],
        })
        ...
        order_metadata = {
            ...
            "order_composition": {
                ...
                "number_of_users": num_users,
                "booking_type": "multi" if num_users > 1 else "single",
                "per_user_amount": grand_total // num_users if num_users > 0 else grand_total,
            },
            "payment_summary": {
                "step_1_base_amounts": {
        ...
        dp_item_metadata = {
            ...
            "daily_pass_pricing": {
                ...
                "actual_per_day_minor": actual_per_day_minor,
                "actual_per_day_rupees": actual_per_day_minor / 100,
                "per_day_per_user_minor": per_day_minor,
                ...
                "per_user_total_minor": dp_total // num_users if num_users > 0 else dp_total,
            },
            "head_count_breakdown": {
                "num_users": num_users,
                "friends_count": friends_count,
                "user_amount_minor": dp_pricing["user_amount"],
                "friends_amount_minor": dp_pricing["friends_amount"],
                "per_friend_amount_minor": dp_pricing["per_friend_amount"],
                "offer_days_count": dp_pricing["offer_days_count"],
                "offer_days_amount": dp_pricing["offer_days_amount"],
                "non_offer_days_count": dp_pricing["non_offer_days_count"],
                "non_offer_days_amount": dp_pricing["non_offer_days_amount"],
                "show_modal_self": dp_pricing["show_modal_self"],
                "show_modal_friend": dp_pricing["show_modal_friend"],
            },
            "pricing_breakdown": {
        ...
        client_row = (await db.execute(select(Client.contact).where(Client.client_id == int(cid)))).scalar()
        ...
        response = UnifiedCheckoutResponse(
            ...
            perUserAmount=per_user_amount,
            gymId=payload.gymId,
            daysTotal=days_total,
            dates=[d.isoformat() for d in dp_dates],
            numberOfUsers=num_users,
            includesSubscription=payload.includeSubscription,
            displayTitle=f"{days_total} Day Pass" + (f" x {num_users} Users" if num_users > 1 else "") + (" + Membership" if payload.includeSubscription else ""),
            description="Daily pass" + (f" for {num_users} users" if num_users > 1 else "") + (" with app membership" if payload.includeSubscription else ""),
    ...
    async def _verify_async(
        ...
        try:
            ...
            # Snapshot scalars before commit — attributes expire after rollback,
            # and touching the ORM instance then triggers a lazy-load on the
            # dead async session (MissingGreenlet).
            _order_db_id = order.id
            _order_gross_amount = order.gross_amount_minor
            try:
                await db.commit()
            except IntegrityError as e:
                # Race condition: another request already created the records
                if "Duplicate entry" in str(e) or "uq_pass_day_unique_date" in str(e):
                    await db.rollback()
                    logger.info(f"[PROCESSOR_DUPLICATE_DETECTED] Duplicate entry detected for order {_order_db_id}, returning already_processed")
                    return UnifiedVerificationResponse(
                        success=True,
                        payment_captured=True,
                        order_id=_order_db_id,
                        payment_id=request.razorpay_payment_id,
                        daily_pass_activated=True,
                        daily_pass_details=dp_details,
                        subscription_activated=sub_ok,
                        subscription_details=sub_details,
                        total_amount=_order_gross_amount,
                        currency="INR",
                        message="Payment already processed",
                    ).dict()
                await db.rollback()
    ...
    async def _upgrade_verify_async(self, db: AsyncSession, payload_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        await db.commit()
        await self._invalidate_home_booking_cache({old_gym_id, new_gym_id}, client_id=p.client_id)
