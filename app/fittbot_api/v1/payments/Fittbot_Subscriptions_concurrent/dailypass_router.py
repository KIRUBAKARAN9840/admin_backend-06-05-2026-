...
async def enqueue_dailypass_checkout(
    ...
    await track_event(
        ...
        product_details={"days": len(getattr(body, "dates", [])), "number_of_users": getattr(body, "numberOfUsers", 1)},
            ...
            for p in rows:
                ...
                resp.append(
                    PassSummary(
                        ...
                        head_count=getattr(p, "head_count", 1) or 1,
                        booking_type=getattr(p, "booking_type", "single") or "single",
                        selected_dates=getattr(p, "selected_dates", None),
                        per_user_price=(getattr(p, "per_user_price", None) or 0) / 100 if getattr(p, "per_user_price", None) else None,
                    )
                )
