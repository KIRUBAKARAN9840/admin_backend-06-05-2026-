...
async def get_monthly_gst_tds_optimized(
    ...
    # Daily Pass: commission and TDS
    _, daily_pass_comm, _, daily_pass_tds = calculate_daily_pass_session_payout(daily_pass_revenue)
    daily_pass_gst = float(Decimal(str(daily_pass_comm)) * Decimal("0.18"))

    # Sessions: commission and TDS
    _, sessions_comm, _, sessions_tds = calculate_daily_pass_session_payout(sessions_revenue)
