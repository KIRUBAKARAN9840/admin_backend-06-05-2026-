...
async def confirm_upload(
    ...
    try:
        ...
        # Look up nutritionist_id associated with current admin contact number
        nutritionist_query = select(Nutritionist.id).where(
            and_(
        ...
        nutritionist_id = nutritionist_result.scalar()

        if not nutritionist_id:
            # Fallback to inactive just in case
            nutritionist_query_fallback = select(Nutritionist.id).where(
                Nutritionist.contact == admin.contact_number
            )
            nutritionist_result_fallback = await db.execute(nutritionist_query_fallback)
            nutritionist_id = nutritionist_result_fallback.scalar()

            if not nutritionist_id:
                raise HTTPException(
                    status_code=400,
                    detail="Logged-in admin is not registered as a nutritionist"
                )
