...
async def save_consultation_form(
    ...
    try:
        ...
        # Get the nutritionist record linked to this admin via contact number
        nutri_query = select(Nutritionist).where(Nutritionist.contact == admin.contact_number)
        nutri_result = await db.execute(nutri_query)
        nutritionist = nutri_result.scalars().first()
        
        if not nutritionist:
            raise HTTPException(status_code=403, detail="Current user is not registered as a nutritionist")
        ...
        data["nutritionist_id"] = nutritionist.id
