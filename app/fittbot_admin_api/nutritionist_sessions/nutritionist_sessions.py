...
from app.models.nutrition_models import Nutritionist, NutritionBooking, CompletedSession, DietTemplate, ClientDietTemplate
...
async def get_calendar_counts(
    ...
    try:
        # First, find the nutritionist by contact number
        # We need to get the nutritionist info regardless of bookings
        nutritionist_query = select(
            Nutritionist.id.label('nutritionist_id'),
            Nutritionist.full_name.label('nutritionist_name'),
            Nutritionist.contact.label('nutritionist_contact')
        ).where(
            and_(
        ...
        nutritionist_rows = nutritionist_result.all()

        if not nutritionist_rows:
            # No nutritionist profile found for this contact number
            return {
                "success": True,
                "data": {
                    "date_counts": {},
        ...
        # Get the nutritionist info
        nutritionist_info = {
            "id": nutritionist_rows[0].nutritionist_id,
            "name": nutritionist_rows[0].nutritionist_name,
            "contact": nutritionist_rows[0].nutritionist_contact
        }

        nutritionist_id = nutritionist_rows[0].nutritionist_id
    ...
    try:
        query = select(
            ...
            and_(
                Nutritionist.contact == admin.contact_number,
                Nutritionist.is_active == True,
                # Show sessions if either:
                # 1. Originally booked on target_date, OR
                # 2. Rescheduled to target_date (rescheduled_at date matches)
                or_(
...
async def reschedule_session(
    ...
    try:
        # Verify the nutritionist owns this booking
        query = select(NutritionBooking).select_from(
            Nutritionist
        ).join(
            NutritionBooking,
            NutritionBooking.nutritionist_id == Nutritionist.id
...
async def complete_session(
    ...
    try:
        ...
        # Fetch the booking and verify nutritionist ownership
