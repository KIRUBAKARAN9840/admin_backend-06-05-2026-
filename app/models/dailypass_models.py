...
class DailyPass(DailyPassBase):
    ...
    selected_dates = Column(MYSQL_JSON, nullable=True)  # JSON list of selected date strings ["2026-03-15","2026-03-18",...]
    ...
    # Multi-user booking fields
    booking_type = Column(String(16), nullable=False, default="single")  # single|multi
    head_count = Column(Integer, nullable=False, default=1)  # number of users in booking
    per_user_price = Column(Integer, nullable=True)  # per-user total price in minor units (paisa)
...
class PromoCampaign(DailyPassBase):
    """
    Reusable promo campaign codes (e.g. BMSOFF) with a global redemption cap.
    """
    __tablename__ = "promo_campaigns"
    __table_args__ = {"schema": "dailypass"}

    id = Column(String(40), primary_key=True, default=lambda: new_id("prc"))
    code = Column(String(50), nullable=False, unique=True, index=True)
    max_redemptions = Column(Integer, nullable=False, default=500)
...
class PromoCampaignRedemption(DailyPassBase):
    """
    Tracks which user redeemed which campaign, linking to the created daily pass.
    """
    __tablename__ = "promo_campaign_redemptions"
    __table_args__ = (
        UniqueConstraint("campaign_id", "client_id", name="uq_campaign_client"),
        {"schema": "dailypass"},
    )

    id = Column(String(40), primary_key=True, default=lambda: new_id("pcr"))
    campaign_id = Column(String(40), ForeignKey("dailypass.promo_campaigns.id"), nullable=False, index=True)
    client_id = Column(String(64), nullable=False, index=True)
    gym_id = Column(String(64), nullable=False)
    daily_pass_id = Column(String(40), ForeignKey("dailypass.daily_passes.id"), nullable=False)
