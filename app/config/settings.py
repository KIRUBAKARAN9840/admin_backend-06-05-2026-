...
class Settings(BaseSettings):
    ...
    revenuecat_webhook_secret: str = Field(env="REVENUECAT_WEBHOOK_SECRET")
    ...
    ai_rate_limit_per_day: int = Field(default=100, env="AI_RATE_LIMIT_PER_DAY")

    # OTP Security - Brute Force Protection
    otp_max_verify_attempts: int = Field(default=5, env="OTP_MAX_VERIFY_ATTEMPTS")
    otp_send_limit_per_phone_per_hour: int = Field(default=5, env="OTP_SEND_LIMIT_PER_PHONE_PER_HOUR")
    otp_send_limit_per_ip_per_hour: int = Field(default=20, env="OTP_SEND_LIMIT_PER_IP_PER_HOUR")
    otp_test_numbers: str = Field(default="", env="OTP_TEST_NUMBERS")
    ...
    # Daily membership offer: flat ₹ discount on even-date days
    membership_daily_offer_enabled: bool = Field(default=True, env="MEMBERSHIP_DAILY_OFFER_ENABLED")
    membership_daily_offer_discount: int = Field(default=100, env="MEMBERSHIP_DAILY_OFFER_DISCOUNT")

    # Dynamic pricing: per-client markup
    reduced_markup_percent: int = Field(default=5, env="REDUCED_MARKUP_PERCENT")
    reduced_markup_client_ids: str = Field(default="", env="REDUCED_MARKUP_CLIENT_IDS")
    ...
    def cors_origins_resolved(self) -> List[str]:
        ...
        trusted_domains = [
            "https://admin.fymble.app","https://payments.fymble.app","https://admin.fittbot.com","https://telecaller.fittbot.com","https://telecaller.fymble.app"
        ]
    ...
    def redis_url_resolved(self) -> str:
        """Single source of truth for Redis URL across all modules."""
        if self.redis_url:
    ...
    def redis_host_resolved(self) -> str:
        """Redis host for modules that need host/port separately."""
        if self.redis_host:
            return self.redis_host
        return "localhost"

    @property
    def redis_port_resolved(self) -> int:
        """Redis port."""
